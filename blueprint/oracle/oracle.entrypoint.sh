...
ORACLE_HOME="${ORACLE_HOME:-/u01/app/oracle/product/12.1.0/xe}"

envsubst < /docker-entrypoint-initdb.d/00-user.sql.template > /docker-entrypoint-initdb.d/00-user.sql
...
mkdir -p /tmp/oracle-load
chown oracle:dba /tmp/oracle-load
chmod 0777 /tmp/oracle-load
...
case "$1" in
	'')
		...
		if [ "$(ls -A /u01/app/oracle/oradata 2>/dev/null)" ]; then
			...
			ln -s /u01/app/oracle/dbs "$ORACLE_HOME/dbs"
			#Startup Database
			...
			set +e
			mv "$ORACLE_HOME/dbs" /u01/app/oracle/dbs
			set -e
		...
		if [ $IMPORT_FROM_VOLUME == "true" ]; then
			...
			for f in $(ls /docker-entrypoint-initdb.d/*); do
				...
				case "$f" in
					...
					*.sql)    echo "[IMPORT] $0: running $f"; echo "exit" | su oracle -c "$CHARSET_MOD $ORACLE_HOME/bin/sqlplus -S / as sysdba @$f" && rm -rf /docker-entrypoint-initdb.d/$f; echo ;;
					*.dmp)    echo "[IMPORT] $0: running $f"; impdp $f ;;
					*.ctl)	  echo "[IMPORT] $0: running $f"
							ctl_file=$(basename $f);
						  	fullname="${ctl_file%.ctl}";
						  	name="${fullname##*-}";

						  	su oracle -c "$CHARSET_MOD $ORACLE_HOME/bin/sqlldr userid='${ORACLE_USERNAME}/${ORACLE_PASSWORD}' control=/docker-entrypoint-initdb.d/$ctl_file data=/docker-entrypoint-initdb.d/$name.csv log=/tmp/oracle-load/$name.log bad=/tmp/oracle-load/$name.bad";;
